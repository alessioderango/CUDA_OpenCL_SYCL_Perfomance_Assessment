  ...
  for (int step = 0; step < steps; step++)
  {
      ...
      Q.submit([&](handler& h) {
          ...
          h.parallel_for<class Compute>(nd_range<2>(range<2>(NDRr,NDRc), range<2>(NDRr_L,NDRc_L)), [=](nd_item<2> idx) {
            ...
            m = calGetMatrixElement(a_d_Sh, c, i, j) - p_epsilon;
            u[0] = calGetMatrixElement(a_d_Sz, c, i, j) + p_epsilon;
            z = calGetMatrixElement(a_d_Sz, c, i + Xi[1], j + Xj[1]);
            h = calGetMatrixElement(a_d_Sh, c, i + Xi[1], j + Xj[1]);
            u[1] = z + h;                                         
            z = calGetMatrixElement(a_d_Sz, c, i + Xi[2], j + Xj[2]);
            h = calGetMatrixElement(a_d_Sh, c, i + Xi[2], j + Xj[2]);
            u[2] = z + h;                                         
            z = calGetMatrixElement(a_d_Sz, c, i + Xi[3], j + Xj[3]);
            h = calGetMatrixElement(a_d_Sh, c, i + Xi[3], j + Xj[3]);
            u[3] = z + h;                                         
            z = calGetMatrixElement(a_d_Sz, c, i + Xi[4], j + Xj[4]);
            h = calGetMatrixElement(a_d_Sh, c, i + Xi[4], j + Xj[4]);
            u[4] = z + h;
            ...
            if (!eliminated_cells[1]) calSetBufferedMatrixElement(a_d_Sf, r, c, 0, i, j, (average - u[1]) * p_r);
            if (!eliminated_cells[2]) calSetBufferedMatrixElement(a_d_Sf, r, c, 1, i, j, (average - u[2]) * p_r);
            if (!eliminated_cells[3]) calSetBufferedMatrixElement(a_d_Sf, r, c, 2, i, j, (average - u[3]) * p_r);
            if (!eliminated_cells[4]) calSetBufferedMatrixElement(a_d_Sf, r, c, 3, i, j, (average - u[4]) * p_r);	
          });
      });
      ...
      Q.submit([&](handler& h) {
          ...
          h.parallel_for<class Update>(nd_range<2>(range<2>(NDRr,NDRc), range<2>(NDRr_L,NDRc_L)), [=](nd_item<2> idx) {
            ...
            double h_next;
            h_next = calGetMatrixElement(a_d_Sh, c, i, j);
            h_next += calGetBufferedMatrixElement(a_d_Sf, r, c, 3, i+Xi[1], j+Xj[1]) - calGetBufferedMatrixElement(a_d_Sf, r, c, 0, i, j);
            h_next += calGetBufferedMatrixElement(a_d_Sf, r, c, 2, i+Xi[2], j+Xj[2]) - calGetBufferedMatrixElement(a_d_Sf, r, c, 1, i, j);
            h_next += calGetBufferedMatrixElement(a_d_Sf, r, c, 1, i+Xi[3], j+Xj[3]) - calGetBufferedMatrixElement(a_d_Sf, r, c, 2, i, j);
            h_next += calGetBufferedMatrixElement(a_d_Sf, r, c, 0, i+Xi[4], j+Xj[4]) - calGetBufferedMatrixElement(a_d_Sf, r, c, 3, i, j);

            calSetMatrixElement(a_d_Sh, c, i, j, h_next);          	
          });
      });
   }
   Q.wait();
