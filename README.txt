...
5) Run the run_profile.sh script by the "bash run_profile.sh -gpu GPUNUMBER -block_size BLOCKSIZE" command by replacing GPUNUMBER with the ID of the output od nvidia-smi of the A100 GPU and BLOCKSIZE with the size of the block, we used a default 16 blocksize for our experiments (please use the same value)

6) Run the run_profile_with_halo.sh script by the "bash run_profile_with_halo.sh -gpu GPUNUMBER -block_size BLOCKSIZE" command by replacing GPUNUMBER with the ID of the output of nvidia-smi of the A100 GPU and BLOCKSIZE with the size of the block, we used a default 16 blocksize for our experiments (please use the same value)

7) Run the run_profile_without_halo.sh script by the "bash run_profile_without_halo.sh -gpu GPUNUMBER -block_size BLOCKSIZE" command by replacing GPUNUMBER with the ID of the output od nvidia-smi of the A100 GPU and BLOCKSIZE with the size of the block, we used a default 16 blocksize for our experiments (please use the same value)

8) Run the run_profile_stresstest.sh script by the "bash run_profile_stresstest.sh -gpu GPUNUMBER -block_size BLOCKSIZE" command by replacing GPUNUMBER with the ID of the output od nvidia-smi of the A100 GPU and BLOCKSIZE with the size of the block, we used a default 16 blocksize for our experiments (please use the same value)

9) Run the run_profile_stresstest_with_halo.sh script by the "bash run_profile_stresstest_with_halo.sh -gpu GPUNUMBER -block_size BLOCKSIZE" command by replacing GPUNUMBER with the ID of the output od nvidia-smi of the A100 GPU and BLOCKSIZE with the size of the block, we used a default 16 blocksize for our experiments (please use the same value)

10) Run the run_profile_stresstest_without_halo.sh script by the "bash run_profile_stresstest_without_halo.sh -gpu GPUNUMBER -block_size BLOCKSIZE" command by replacing GPUNUMBER with the ID of the output od nvidia-smi of the A100 GPU and BLOCKSIZE with the size of the block, we used a default 16 blocksize for our experiments (please use the same value)
