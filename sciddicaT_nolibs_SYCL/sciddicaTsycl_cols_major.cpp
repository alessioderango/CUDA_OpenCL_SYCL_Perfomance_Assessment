...
// // #define calSetMatrixElement(M, rows, i, j, value) ((M)[(((j) * (rows)) + (i))] = (value))
// // #define calGetMatrixElement(M, rows, i, j) (M[(((j) * (rows)) + (i))])
// // #define calSetBufferedMatrixElement(M, rows, columns, n, i, j, value) ( (M)[( ((n)*(rows)*(columns)) + ((j)*(rows)) + (i) )] = (value) )
// // #define calGetBufferedMatrixElement(M, rows, columns, n, i, j) ( M[( ((n)*(rows)*(columns)) + ((j)*(rows)) + (i) )] )
...
//       // reset kernel
//       Q.submit([&](handler& h) {
//           h.parallel_for<class Reset>(nd_range<2>(range<2>(NDRc,NDRr), range<2>(NDRc_L,NDRr_L)), [=](nd_item<2> idx) {

//             // int i = idx[1];
//             // int j = idx[0];
//             int j = idx.get_global_id(0);
//             int i = idx.get_global_id(1);
...
//       Q.submit([&](handler& h) {
//           h.parallel_for<class Compute>(nd_range<2>(range<2>(NDRc,NDRr), range<2>(NDRc_L,NDRr_L)), [=](nd_item<2> idx) {
//             // int i = idx[1];
//             // int j = idx[0];
//             int j = idx.get_global_id(0);
//             int i = idx.get_global_id(1);
...
//       // balance kernel
//       Q.submit([&](handler& h) {
//           h.parallel_for<class Update>(nd_range<2>(range<2>(NDRc,NDRr), range<2>(NDRc_L,NDRr_L)), [=](nd_item<2> idx) {
//             // int i = idx[1];
//             // int j = idx[0];
//             int j = idx.get_global_id(0);
//             int i = idx.get_global_id(1);
...
//             double h_next;
//             h_next = calGetMatrixElement(d_Sh, r, i, j);
