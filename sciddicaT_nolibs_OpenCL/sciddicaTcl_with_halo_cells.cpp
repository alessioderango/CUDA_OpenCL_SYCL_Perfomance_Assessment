...
#define COMPUTING_THREADS_PER_BLOCK (THREADS_PER_BLOCK-2)

#define MASK_WIDTH (3)
#define INPUT_TILE_WIDTH (COMPUTING_THREADS_PER_BLOCK + MASK_WIDTH - 1)
#define OUTPUT_TILE_WIDTH (COMPUTING_THREADS_PER_BLOCK)
...
int main(int argc, char **argv)
{
  ...
  int THREADS_PER_BLOCK = atoi(argv[BLOCK_SIZE_ID]);
  ...
  try
  {
    ...
    cl::Context context(device);
    std::string flags = "-DTILE_SIZE="+std::to_string(THREADS_PER_BLOCK);
    std::cout << flags << std::endl;
    // Load in kernel source, creating a program object for the context
   // std::string flags2 ="";//= "-cl-no-signed-zeros -cl-strict-aliasing -cl-mad-enable -cl-unsafe-math-optimizations -cl-finite-math-only -cl-fast-relaxed-math";
    cl::Program p_reset(context, util::loadProgram("./kernel_with_halo_cells/k_reset_local.cl"), false);
    p_reset.build(flags.c_str());
    std::cout << "compiled Reset" << std::endl;

    cl::Program p_computation(context, util::loadProgram("./kernel_with_halo_cells/k_computation_local.cl"), false);
    p_computation.build(flags.c_str());
    std::cout << "compiled p_computation" << std::endl;

    cl::Program p_balance(context, util::loadProgram("./kernel_with_halo_cells/k_balance_local.cl"), false);
    p_balance.build(flags.c_str());
    ...
    int NDRr_L = INPUT_TILE_WIDTH;
    int NDRc_L = INPUT_TILE_WIDTH;
    int NDRr = INPUT_TILE_WIDTH * ceil( (float)r / OUTPUT_TILE_WIDTH);
    int NDRc = INPUT_TILE_WIDTH * ceil( (float)c / OUTPUT_TILE_WIDTH);
    ...
    int NDRr_L_reset = THREADS_PER_BLOCK;
    int NDRc_L_reset = THREADS_PER_BLOCK;
    ...
    cl::NDRange global_reset(NDRr_reset, NDRc_reset);
    cl::NDRange local_reset(NDRr_L_reset, NDRc_L_reset);
    ...
    {
      k_reset( cl::EnqueueArgs( queue, global_reset, local_reset), r, c, nodata, /*d_Sz,*/ d_Sf);
