...
int main(int argc, char **argv)
{
  ...
  for (int step = 0; step < steps; step++)
  {
          ...
          h.parallel_for<class Compute>(nd_range<2>(range<2>(NDRr,NDRc), range<2>(NDRr_L,NDRc_L)), [=](nd_item<2> idx) {
            ...
            int Xj[] = {0,  0, -1,  1,  0};
	          ...
	          if ((GLOBAL_THREAD_ROW_INDEX_WITH_OFFSET < r) && (GLOBAL_THREAD_COLUMN_INDEX_WITH_OFFSET < c))
	          {
	          	...
	          	const double u_zero =  calGetMatrixElement(d_Sz, c, GLOBAL_THREAD_ROW_INDEX_WITH_OFFSET, GLOBAL_THREAD_COLUMN_INDEX_WITH_OFFSET)
                                     + p_epsilon;
	          	u_neighbour_ds[iLocal][jLocal] = 
                  calGetMatrixElement(d_Sz, c, GLOBAL_THREAD_ROW_INDEX_WITH_OFFSET, GLOBAL_THREAD_COLUMN_INDEX_WITH_OFFSET) + 
                  calGetMatrixElement(d_Sh, c, GLOBAL_THREAD_ROW_INDEX_WITH_OFFSET, GLOBAL_THREAD_COLUMN_INDEX_WITH_OFFSET);

	          	const double initial_average = calGetMatrixElement(d_Sh, c, GLOBAL_THREAD_ROW_INDEX_WITH_OFFSET, GLOBAL_THREAD_COLUMN_INDEX_WITH_OFFSET) - p_epsilon;
	          	...
	          	for (neighbour_index = 1; neighbour_index < 5; ++neighbour_index)
	          	{
	          		...
	          		)
	          		{
	          			...
	          			calGetMatrixElement(d_Sz, c, GLOBAL_THREAD_ROW_INDEX_WITH_OFFSET + Xi[neighbour_index], GLOBAL_THREAD_COLUMN_INDEX_WITH_OFFSET + Xj[neighbour_index])
	          				+ calGetMatrixElement(d_Sh, c, GLOBAL_THREAD_ROW_INDEX_WITH_OFFSET + Xi[neighbour_index], GLOBAL_THREAD_COLUMN_INDEX_WITH_OFFSET + Xj[neighbour_index]);
	          		}
	          	}
	            idx.barrier(sycl::access::fence_space::local_space);
	          	...
	          	for (neighbour_index = 1; neighbour_index < 5; ++neighbour_index)
	          	{
	          		if (!eliminated_cells_ds[neighbour_index])
	          		{
	          			...
	          			{
                     calSetBufferedMatrixElement(d_Sf, r, c,  (neighbour_index - 1), GLOBAL_THREAD_ROW_INDEX_WITH_OFFSET, GLOBAL_THREAD_COLUMN_INDEX_WITH_OFFSET, (average - u_neighbour_ds[iLocal + Xi[neighbour_index]][jLocal + Xj[neighbour_index]]) * p_r);
	          			}
	          			else
	          			{
                     calSetBufferedMatrixElement(d_Sf, r, c, (neighbour_index - 1), GLOBAL_THREAD_ROW_INDEX_WITH_OFFSET, GLOBAL_THREAD_COLUMN_INDEX_WITH_OFFSET, (average - u_neighbour[neighbour_index - 1]) * p_r);
	          			}
	          		}
	          	}
	          }
      ...
      // balance kernel
      Q.submit([&](handler& h) { 
          accessor<double, 3, access::mode::read_write,access::target::local> Sf_ds(sycl::range<3>(NDRr_L,NDRc_L,4), h);
          h.parallel_for<class Update>(nd_range<2>(range<2>(NDRr,NDRc), range<2>(NDRr_L,NDRc_L)), [=](nd_item<2> idx) {
            ...
            for (int neighbour_index = 0; neighbour_index < ADJACENT_CELLS; ++neighbour_index)
	          {
                  ...
                  calGetBufferedMatrixElement(d_Sf, r, c, neighbour_index, GLOBAL_THREAD_ROW_INDEX_WITH_OFFSET, GLOBAL_THREAD_COLUMN_INDEX_WITH_OFFSET);
	          	}
	          	else
	          	{
            ...
            if
	          ...
	          {
	          	double h_next = calGetMatrixElement(d_Sh, c, GLOBAL_THREAD_ROW_INDEX_WITH_OFFSET, GLOBAL_THREAD_COLUMN_INDEX_WITH_OFFSET);
                  ...
                  (GLOBAL_THREAD_ROW_INDEX_WITH_OFFSET + Xi[neighbour_index + 1] >= (      idx.get_group(1)      * NDRr_L + OFFSET))
	          		...
	          		{
	          			h_next += calGetBufferedMatrixElement(d_Sf, r, c,  (3 - neighbour_index), GLOBAL_THREAD_ROW_INDEX_WITH_OFFSET + Xi[neighbour_index + 1], GLOBAL_THREAD_COLUMN_INDEX_WITH_OFFSET + Xj[neighbour_index + 1]);
	          		}

	          		h_next -= Sf_ds[iLocal][jLocal][neighbour_index];
	          	}

	          	calSetMatrixElement(d_Sh, c, GLOBAL_THREAD_ROW_INDEX_WITH_OFFSET, GLOBAL_THREAD_COLUMN_INDEX_WITH_OFFSET, h_next);
	          }
