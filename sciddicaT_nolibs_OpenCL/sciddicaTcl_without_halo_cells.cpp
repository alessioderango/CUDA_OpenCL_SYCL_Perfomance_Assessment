...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    std::string flags = "-DTILE_WIDTH_TILING_WIDTH="+std::to_string(THREADS_PER_BLOCK)+" -DTHREADS_PER_BLOCK_TILING_WIDTH_UPDATE="+std::to_string(THREADS_PER_BLOCK);
    std::cout << flags << std::endl;
    //char* flags ="-DTILE_WIDTH_TILING_WIDTH=";// "-cl-no-signed-zeros -cl-strict-aliasing -cl-mad-enable -cl-unsafe-math-optimizations -cl-finite-math-only -cl-fast-relaxed-math";
    cl::Program p_reset(context, util::loadProgram("./kernel_without_halo_cells/k_reset_local.cl"), false);
    p_reset.build(flags.c_str());
    cl::Program p_computation(context, util::loadProgram("./kernel_without_halo_cells/k_computation_local.cl"), false);
    p_computation.build(flags.c_str());
    cl::Program p_balance(context, util::loadProgram("./kernel_without_halo_cells/k_balance_local.cl"), false);
    ...
    int NDRr_L = THREADS_PER_BLOCK;
    int NDRc_L = THREADS_PER_BLOCK;
    ...
    cl::NDRange global(NDRc, NDRr);
    cl::NDRange local(NDRc_L, NDRr_L);
